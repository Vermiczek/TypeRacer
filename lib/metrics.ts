 ...
 */
export const calcWpm = (correctChars: number, elapsedSeconds: number): number => {
 ...
 */
export const calcAccuracy = (typed: string, target: string): number => {
 ...
 */
export const countCorrectChars = (typed: string, target: string): number => {
  let count = 0;
  for (let i = 0; i < typed.length; i++) {
...
export const getCharStates = (typed: string, target: string): CharState[] => {
